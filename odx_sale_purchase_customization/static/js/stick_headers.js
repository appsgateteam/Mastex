odoo.define('odx_sale_purchase_customization.stick_header', function (require) {
'use strict';
    var ListView = require('web.ListRenderer');
    ListView.include({

    _freezeColumnWidths: function () {
            if(this.getParent().$el.hasClass("o_field_many2many") !== false) {
                ...
                if(this.$el.find('.table.o_list_table').length!==0){
                    sticky();
                    fix_body("relative");
                    $(window).unbind('resize', sticky).bind('resize', sticky);
                     this.$el.css("overflow-x","visible");
